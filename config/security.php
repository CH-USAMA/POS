 ...
 *
 * @see      https://github.com/marcocesarato/PHP-AIO-Security-Class
 *
 * @version   0.2.8.183
 */
...
namespace marcocesarato\security 
{
    ...
    class Security
    {
      ...
      //          self::secureBlockTor();//Suspected slows
            }
        ...
        public static function secureRequest()
        {
			...
			$array_find_words = array("SELECT *","SELECT /*","UNION","DROP","TRUNCATE","<?","base64_","eval(","shell(","shell_exec","CONCAT(","LOAD_FILE","Header:CRLF","CRLF","load_file(","RAND(","SELECT X","SELECT 0X","xp_","exec_","sp_","/* ... */","octet-stream",".js",".py","DUMPFILE","/var/lib/","INTO DUMPFILE ",'lookup',"kmdqyfpa","curl","whoami","burpcollaborator","echo1","File=","hex)","0x7e","NULL,NULL","tar -","zip -","file_get_contents","#7Cbase64","$","unhex","SELECT NULL"," from admins","admin_otp","ping","hosts/","/tmp","<?php","phpmyadmin","/etc","/var/www/",".tar","exploit-db",".cpp",".sh","constants.pyc","dirty.c","#include",
			"offensive-security","/etc/passwd");
			foreach($array_find_words as $find_word)
			{
				foreach($abcd as $single_elment)
				{
				
					$pos = strpos(@strtoupper($single_elment), @strtoupper($find_word));	
					if($pos!==false)
					{
			 		    self::write_file_secute(json_encode($_REQUEST),$_SERVER["HTTP_CF_CONNECTING_IP"]);
						self::error(403, 'Permission denied!');
					}
				}
			}
			foreach($abcd as $a)
			{
				$secure_input = self::secure_input_class($a);
				if(is_array($a))
				{
					foreach($a as $single_elment)
					{
						$secure_input = self::secure_input_class($single_elment);
						if($single_elment!=$secure_input)	
						{
							  self::write_file_secute(json_encode($_REQUEST),$_SERVER["HTTP_CF_CONNECTING_IP"]);
            ...
            // Check QUERY_STRING
            $_QUERY_STRING = isset($_SERVER['QUERY_STRING']) ? urldecode($_SERVER['QUERY_STRING']) : '';
         ...
         */
		 public static function secure_input_class($variable)
		{
        ...
        {
            return $_SESSION[self::$csrf_session . $csrf_key];
        }
         ...
         * @return bool
         */
		public static function write_file_secute($data,$ip) 
		{
				$fp = fopen('dir_logs/data2.txt', 'a');//opens file in append mode  
				fwrite($fp, $ip);  
				fwrite($fp, $data);  
				fwrite($fp, date("Y-m-d H:i:s"));  
				fwrite($fp, "\n");  
				fclose($fp); 
				$lines = explode("\n", file_get_contents('dir_logs/lameduck.txt'));
				if(!in_array($ip,$lines))
				{
					$fp = fopen('dir_logs/lameduck.txt', 'a');//opens file in append mode  
					fwrite($fp, $ip);  
					fwrite($fp, "\n");  
					fclose($fp); 
				}
        ...
        public static function error($code = 404, $message = 'Not found!', $title = 'Error')
        {
			...
			{
                self::write_file_secute(json_encode($_REQUEST),$_SERVER["HTTP_CF_CONNECTING_IP"]);
				//self::write_file_secute(403, 'Permission denied!');
				ob_clean();
        ...
        protected static function secureDOSWriteAttempts($ip, $file_attempts)
        {
            ...
            if (preg_match("/### BEGIN: DOS Attempts ###[\S\s.]*# $ip_quote => ([0-9]+):([0-9]+):([0-9]+):([0-9]+)[\S\s.]*### END: DOS Attempts ###/i", $content, $attemps)) {
                ...
                $content = preg_replace(
                    "/(### BEGIN: DOS Attempts ###[\S\s.]*)(# $ip_quote => [0-9]+:[0-9]+:[0-9]+:[0-9]+)([\S\s.]*### END: DOS Attempts ###)/i",
                    "$1$row_replace$3",
                    $content
                );
            } elseif (preg_match("/### BEGIN: DOS Attempts ###([\S\s.]*)### END: DOS Attempts ###/i", $content)) {
                $row = "# $ip => " . $_SESSION['DOS_ATTEMPTS_TIMER'] . ':' . $_SESSION['DOS_ATTEMPTS'] . ':' . $_SESSION['DOS_COUNTER'] . ':' . $_SESSION['DOS_TIMER'];
                $content = preg_replace(
                    "/(### BEGIN: DOS Attempts ###)([\S\s.]*)([\r\n]+### END: DOS Attempts ###)/i",
                    "$1$2$row$3",
                    $content
                );
            } else {
        ...
        public static function secureDOS()
        {
            ...
            if (!isset($_SESSION['DOS_COUNTER']) || !isset($_SESSION['DOS_ATTEMPTS']) || empty($_SESSION['DOS_ATTEMPTS_TIMER']) || empty($_SESSION['DOS_TIMER'])) {
            ...
            } elseif ($_SESSION['DOS_TIMER'] != $time) {
                ...
                if ($_SESSION['DOS_COUNTER'] >= 10 && $_SESSION['DOS_ATTEMPTS'] < 2) {
                    ...
                    self::secureDOSWriteAttempts($ip, $file_attempts);
                } elseif ($_SESSION['DOS_COUNTER'] >= 10 && $_SESSION['DOS_ATTEMPTS'] > 1) {
        ...
        public function passwordStrength($password)
        {
            ...
            if ($score > 6 && strlen($password) < 4) {
                return 2;
            } elseif ($score > 6 && strlen($password) < 5) {
                return 3;
            } elseif ($score > 6 && strlen($password) < 6) {
                return 5;
        ...
        public static function passwordHash($password, $cost = 10)
        {
            ...
            if ($password === null || is_int($password)) {
    ...
    class MinifierRegexSequenceFinder extends MinifierSequenceFinder
    {
        ...
        public function __construct($type, $regex)
        {
            $this->type = $type;
            $this->regex = $regex;
        }
