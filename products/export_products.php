...
header('Content-Disposition: attachment; filename="products_lists.csv"');


$sWhere = "SELECT products.id as id,products.name as name,categories.name as category_name,products.created_at as created_at from products,categories  where categories.id = products.category_id";
...
if($numRowsCheckQryTotal > 0) {
	...
	$my_arraryHeader = array('Product Name','Category Name', 'Created at');
